...
**Output:** Test counts before/after in the commit body.
...
|---|---|---|
| API handler (P95) | < 100ms cold, < 30ms warm (excl. provider call) | `pytest-benchmark` or in-test `time.perf_counter`. |
| DB query (single) | < 10ms on the local Postgres | `EXPLAIN ANALYZE` for any new query touching > 1 row. |
| Page first-meaningful-paint | < 1.5s on the local stack | `mcp__plugin_playwright_playwright__browser_take_screenshot` + `Date.now()` markers. |
| Bundle size delta | < +20kb gzipped per route | `vite build` output. |

**If the bar isn't hit:** Stop, profile, fix. Do not ship-and-fix later.

**Output:** Numbers in the commit body. If the change is performance-neutral by design (docs, comments), say so explicitly.
...
* **Self-review second:** read the diff one more time before commit, looking for the specific issues the agent flagged.
...
| Internal core wiring (no UI) | Stage 5 has no Playwright. |
...
| New endpoint | All nine stages run in full. |
| New web route | All nine stages, with Playwright mandatory in stage 5. |
